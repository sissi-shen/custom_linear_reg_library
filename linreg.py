...
def minimize(X, y, loss_gradient,
    ...
    historical_gradients = np.ones((p,1))
    for i in range(max_iter):
        gradient = loss_gradient(X, y, B, lmbda)
        historical_gradients += gradient**2
        B = B - (eta / np.sqrt(historical_gradients + eps)) * gradient
...
class RidgeRegression621: # MODIFY THIS ONE
    ...
    def fit(self, X, y):
        # Remember here that you need to estimate B0 seeparately
